...
class TestEnrichImportLogic:
    async def test__nominal(self, user: User, async_session_db: AsyncSession) -> None:
        ...
        data = [{"fingerprint": track_db.fingerprint, "genres": ["indie folk"], "moods": ["melancholic"]}]
        ...
        assert updated.genres == ["indie folk"]
        assert updated.moods == ["melancholic"]

    async def test__reimport_overrides(self, user: User, async_session_db: AsyncSession) -> None:
        track_db = await TrackModelFactory.create_async(user_id=user.id, genres=["pop"], moods=["happy"])

        await import_logic(
            email=user.email, data=[{"fingerprint": track_db.fingerprint, "genres": ["rock"], "moods": []}]
        )
        await import_logic(
            email=user.email, data=[{"fingerprint": track_db.fingerprint, "genres": ["indie"], "moods": ["calm"]}]
        )

        stmt = select(TrackModel).where(TrackModel.id == track_db.id)
        updated = (await async_session_db.execute(stmt)).scalar_one()
        assert updated.genres == ["indie"]
        assert updated.moods == ["calm"]
