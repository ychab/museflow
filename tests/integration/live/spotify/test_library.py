...
class TestSpotifyLibraryLive:
    ...
    * connect a user account with Spotify and manually copy its real refresh token from the DB
    ...
    Then execute:
    > SPOTIFY_CLIENT_ID=<REAL_CLIENT_ID> SPOTIFY_CLIENT_SECRET=<REAL_CLIENT_SECRET> uv run pytest ./tests/integration/live/spotify --spotify-refresh-token=<REFRESH_TOKEN> --no-cov
    """
    ...
    @pytest.fixture
    async def spotify_oauth_live(self) -> AsyncGenerator[ProviderOAuthPort]:
    ...
    async def spotify_session_live(
        ...
        spotify_oauth_live: SpotifyOAuthAdapter,
    ) -> SpotifyOAuthSessionClient:
        token_payload = await spotify_oauth_live.refresh_access_token(spotify_refresh_token)
        ...
        return SpotifyOAuthSessionClient(
            ...
            oauth_client=spotify_oauth_live,
        )
