...
class TestRateExportParserCommand:
    @pytest.fixture(autouse=True)
    def mock_export_logic(self) -> Iterable[mock.AsyncMock]:
        target_path = "museflow.infrastructure.entrypoints.cli.commands.rate.export.export_logic"
    ...
    def mock_yaml_dump(self) -> Iterable[mock.Mock]:
        with mock.patch("museflow.infrastructure.entrypoints.cli.commands.rate.export.yaml.safe_dump") as patched:
            yield patched
    ...
    def test__email__invalid(
        ...
        result = runner.invoke(app, ["rate", "export", "--email", email])
...
class TestRateExportCommand:
    ...
    def test__user_not_found(
        ...
        result = runner.invoke(
            app,
            ["rate", "export", "--email", "test@example.com", "--output", str(tmp_path / "out.yaml")],
        )
        assert result.exit_code != 0
    ...
    def test__nominal(
        ...
        entries: list[dict[str, Any]] = [{"fingerprint": "abc123", "score": 8}]
        mock_export_logic.return_value = entries
        output_path = tmp_path / "out.yaml"

        result = runner.invoke(
            app,
            ["rate", "export", "--email", "test@example.com", "--output", str(output_path)],
        )
        assert result.exit_code == 0
...
class TestRateExportLogic:
    TARGET_PATH: Final[str] = "museflow.infrastructure.entrypoints.cli.commands.rate.export"
