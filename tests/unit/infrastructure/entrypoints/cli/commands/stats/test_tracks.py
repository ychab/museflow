...
class TestStatsTracksCommand:
    ...
    def test__with_tracks_table(self, mock_tracks_logic: mock.AsyncMock, runner: CliRunner) -> None:
        track = TrackFactory.build(name="Song A", artists=["Artist X"], score=9)
...
class TestStatsTracksLogic:
    ...
    async def test__user_not_found(self, mock_user_repository: mock.AsyncMock) -> None:
        ...
        with pytest.raises(UserNotFound):
            await tracks_logic(email="test@example.com", limit=20, score_min=None, score_max=None)
    ...
    async def test__score_min_passed_to_repository(
        ...
        await tracks_logic(email=user.email, limit=20, score_min=7, score_max=None)

        mock_track_repository.get_list.assert_awaited_once_with(user_id=user.id, min_score=7, max_score=None, limit=20)
    ...
    async def test__score_min_defaults_to_zero_when_none(
        ...
        await tracks_logic(email=user.email, limit=20, score_min=None, score_max=None)

        mock_track_repository.get_list.assert_awaited_once_with(user_id=user.id, min_score=0, max_score=None, limit=20)
