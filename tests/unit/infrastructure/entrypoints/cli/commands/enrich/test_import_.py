...
class TestEnrichImportParserCommand:
    @pytest.fixture(autouse=True)
    def mock_import_logic(self) -> Iterable[mock.AsyncMock]:
        target_path = "museflow.infrastructure.entrypoints.cli.commands.enrich.import_.import_logic"
        with mock.patch(target_path, autospec=True) as patched:
            patched.return_value = EnrichImportResult(imported_count=0, not_found_count=0)
            yield patched
    ...
    def test__email__invalid(
        ...
        valid_yaml = tmp_path / "enrichments.yaml"
        valid_yaml.write_text("- fingerprint: abc\n  genres: [rock]\n  moods: []\n")

        result = runner.invoke(app, ["enrich", "import", "--email", email, "--input", str(valid_yaml)])
...
class TestEnrichImportCommand:
    ...
    def test__file_not_found(
        ...
        result = runner.invoke(
            app,
            ["enrich", "import", "--email", "test@example.com", "--input", "/nonexistent/path/enrichments.yaml"],
        )
    ...
    def test__yaml_parse_error(
        ...
        result = runner.invoke(
            app,
            ["enrich", "import", "--email", "test@example.com", "--input", str(bad_yaml)],
        )
        assert result.exit_code == 1
    ...
    def test__user_not_found(
        ...
        result = runner.invoke(
            app,
            ["enrich", "import", "--email", "test@example.com", "--input", str(valid_yaml)],
        )
        assert result.exit_code != 0
    ...
    def test__nominal(
        ...
        mock_import_logic.return_value = EnrichImportResult(imported_count=3, not_found_count=1)
        valid_yaml = tmp_path / "enrichments.yaml"
        valid_yaml.write_text("- fingerprint: abc\n  genres: [rock]\n  moods: []\n")
...
class TestEnrichImportLogic:
    TARGET_PATH: Final[str] = "museflow.infrastructure.entrypoints.cli.commands.enrich.import_"
    ...
    async def test__fingerprint_not_found(
        ...
        result = await import_logic(
            email="test@example.com",
            data=[{"fingerprint": "unknown-fp", "genres": ["rock"], "moods": []}],
        )

        assert result.imported_count == 0
        assert result.not_found_count == 1
        mock_track_repository.bulk_update.assert_not_awaited()
    ...
    async def test__nominal(
        ...
        result = await import_logic(
            email="test@example.com",
            data=[{"fingerprint": fingerprint, "genres": ["rock", "indie-rock"], "moods": ["energetic"]}],
        )
        ...
        assert call_args.kwargs["fields"] == frozenset(EnrichField)
