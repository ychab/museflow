...
class TestSpotifyOAuthAdapter:
    ...
    def test__get_authorization_url(self, spotify_oauth: SpotifyOAuthAdapter) -> None:
        ...
        assert query_params["scope"] == [
            "playlist-modify-public playlist-modify-private user-modify-playback-state user-library-modify"
        ]
    ...
    async def test__exchange_code_for_token__nominal(
        ...
        assert token_payload.refresh_token == response_json["refresh_token"]
        assert token_payload.expires_at == frozen_time + timedelta(seconds=response_json["expires_in"])
    ...
    async def test__refresh_access_token__nominal(
        ...
        assert token_payload.token_type == response_json["token_type"]
        assert token_payload.access_token == response_json["access_token"]
        assert token_payload.refresh_token == response_json.get("refresh_token", old_refresh_token)
        assert token_payload.expires_at == frozen_time + timedelta(seconds=response_json["expires_in"])
    ...
    async def test__make_api_call__retry__rate_limit__with_header__exceeds_max_wait(
        self,
        token_payload: OAuthProviderTokenPayload,
        httpx_mock: HTTPXMock,
    ) -> None:
