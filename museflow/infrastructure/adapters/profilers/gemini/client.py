...
class GeminiTasteProfileAdapter(HttpClientMixin, TasteProfilerPort):
    ...
    async def merge_profiles(self, foundation: TasteProfileData, new_segment: TasteProfileData) -> TasteProfileData:
        ...
        prompt = (
            ...
            f"New segment:\n{segment_json}\n\n"
            "Rules:\n"
            "- taste_timeline: if this segment is >80% similar to the last era (same dominant moods AND "
            "technical_fingerprint values differ by <0.15 on average), merge them by extending the time_range "
            "instead of adding a new entry. Hard cap: keep the total timeline under 20 entries. "
