...
def _ensure_list(v: Any) -> list[Any]:
    """
    Last.fm sometimes returns a single object instead of a list when there is
    only one result. This validator ensures we always get a list.
    """
    if v is None:
        return []
...
class LastFmSimilarTracksResponse(BaseModel):
    similartracks: LastFmSimilarTracks | None = None
    error: int | None = None
